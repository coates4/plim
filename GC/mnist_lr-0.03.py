...
plt.plot(loss_all)
plt.savefig('poisoned_models/lr/img/total_loss_{}.png'.format(epsilon))
plt.show()
...
data_p = torch.load('poisoned_models/lr/data_p_{}.pt'.format(epsilon)).to('cpu')
target_p = torch.load('poisoned_models/lr/target_p_{}.pt'.format(epsilon)).to('cpu')
...
def test():
    model1.load_state_dict(torch.load('poisoned_models/lr/poisoned_lr_model_{}.pt'.format(epsilon)))
    model1.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
plt.savefig('poisoned_models/lr/img/retrain_loss_{}.png'.format(epsilon))
plt.show()
